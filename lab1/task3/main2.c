...
int main(int n, char *commands[]) {
    void *handle;
    handle = dlopen("./libtask3.so", RTLD_LAZY);
    *(void**)(&make_array) = dlsym(handle, "make_array");
    *(void**)(&count_text_one_file) = dlsym(handle, "count_text_one_file");
    *(void**)(&count_text) = dlsym(handle, "count_text");
    *(void**)(&write_from_file) = dlsym(handle, "write_from_file");
    *(void**)(&delete_block) = dlsym(handle, "delete_block");
    *(void**)(&are_free_before_ind) = dlsym(handle, "are_free_before_ind");
    int i = 1;
    while(i<n){
